  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col font-sans text-white overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="p-6 border-b border-white/5 flex items-center justify-between bg-[#050505] shadow-2xl">
        ...
        <div className="text-center">
          <h2 className="text-xl font-black uppercase italic tracking-tighter">Hugo <span className="text-green-500">Master Route</span></h2>
          <p className="text-[7px] text-gray-500 font-black uppercase tracking-[0.4em] mt-1 italic">Inteligencia de Abasto Real</p>
        </div>
        <button 
            onClick={() => setShowChecklist(true)} 
            className="relative bg-white/5 p-3 rounded-2xl border border-white/10 hover:border-green-500/50 transition-all"
        >
          <ClipboardList size={20} className={itemsEnLista > 0 ? 'text-green-500' : 'text-gray-500'} />
          {itemsEnLista > 0 && <span className="absolute -top-1 -right-1 bg-green-600 text-[8px] font-black w-4 h-4 rounded-full flex items-center justify-center animate-bounce">{itemsEnLista}</span>}
        </button>
      </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-3 no-scrollbar pb-40">
            ...
            {itemsEnLista === 0 && (
              <div className="py-40 text-center opacity-20">
                <ClipboardList className="mx-auto mb-6" size={80} />
                <p className="text-xs font-black uppercase tracking-[0.4em]">Lista Vacía Socio</p>
              </div>
            )}
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-6 bg-black border-t border-white/10 shadow-[0_-20px_50px_rgba(0,0,0,1)] z-[60]">
        <div className="max-w-7xl mx-auto flex justify-between items-center mb-4 px-2">
          <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Inversión Estimada</p>
          <p className="text-2xl font-black text-white">{formatCurrency(totalInversion)}</p>
        </div>
        <button onClick={ejecutarCompraMaestra} disabled={issubmitting} className="w-full bg-green-600 h-16 rounded-[28px] text-black font-black uppercase text-xs tracking-widest active:scale-95 transition-all shadow-xl shadow-green-900/20">
          {issubmitting ? 'Sincronizando...' : 'Finalizar y Actualizar Precios'}
        </button>
      </div>
    </div>
  );
