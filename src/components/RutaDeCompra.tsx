...
export default function RutaDeCompra({ onBack }: { onBack: () => void }) {
  ...
  // ✅ CEREBRO DE PRIORIDAD Y SEMÁFORO (RESTAURADO)
  const analysis = useMemo(() => {
    return products.map(p => {
      ...
      // Lógica de Urgencia Atómica
      let urg = 3; // Verde (Normal)
  ...
  // ✅ ORDENAMIENTO POR PRIORIDAD DE CATEGORÍAS
  ...
  const ejecutarCompraMaestra = async () => {
    ...
    if (itemsParaComprar.length === 0) return alert("Hugo, marca qué compraste.");
    ...
    <div className="bg-[#050505] min-h-screen pb-48 font-sans text-white">
      ...
      <div className="p-6 bg-black border-b border-white/10 flex justify-between items-center sticky top-0 z-[70] backdrop-blur-xl">
        ...
        <div className="text-center">
          <h2 className="text-xl font-black uppercase italic tracking-tighter">Misión <span className="text-green-500">{ordenPlaneada ? 'Planeada' : 'Libre'}</span></h2>
      ...
      {ordenPlaneada && (
        <div className="mx-6 mt-6 p-4 bg-blue-600/20 border border-blue-500/30 rounded-2xl flex items-center gap-4 animate-pulse">
           ...
           <div><p className="text-[10px] font-black uppercase text-white">Guía de Abasto Cargada</p><p className="text-[8px] font-black text-blue-400 uppercase tracking-widest">Pre-configuramos {ordenPlaneada.items.length} sugerencias.</p></div>
        </div>
      )}
      ...
      <div className="px-6 space-y-4 max-w-7xl mx-auto">
        {sortedCategories.map(cat => {
          ...
          // Ordenar items dentro de la categoría por urgencia (Rojo primero)
            ...
            <div key={cat} className={`border rounded-[35px] overflow-hidden transition-all ${expandedCategory === cat ? 'bg-[#0A0A0A] border-white/10' : 'border-white/5 bg-white/[0.01]'}`}>
              <button onClick={() => setExpandedCategory(expandedCategory === cat ? null : cat)} className="w-full p-6 flex justify-between items-center">
                <div className="flex items-center gap-4">
                  {/* ✅ SEMÁFORO DE CATEGORÍA RESTAURADO */}
              ...
              {expandedCategory === cat && (
                <div className="p-4 space-y-4 bg-black/40">
                  {sortedItems.map(item => {
                    ...
                    return (
                      <div key={item.sku} className={`p-6 rounded-[2.5rem] border ${estaEnPlan ? 'border-blue-500/30 bg-blue-500/5' : item.urg === 0 ? 'border-red-600/20 bg-red-600/[0.02]' : 'border-white/5'}`}>
