...
export default function AdminOrders() {
  ...
  // 1. ACTUALIZACIÓN LOCAL (PARA LA INTERFAZ)
  ...
  // 2. PERSISTENCIA TOTAL (PARA SUPABASE Y WHATSAPP)
  const handleConfirmWeights = async (orderId: number) => {
    // Buscamos la orden con los datos ya editados en la interfaz local
    const orderToUpdate = orders.find(o => o.id === orderId);
    if (!orderToUpdate) return;

    const nextStatus = 'Pendiente de Pago';
    
    // --- ACTUALIZACIÓN DE TRIPLE IMPACTO EN SUPABASE ---
    const { error } = await supabase
      .from('pedidos')
      .update({ 
        estado: nextStatus,
        detalle_pedido: orderToUpdate.detalle_pedido, // Guardamos los pesos reales
        total: orderToUpdate.total                  // Guardamos el dinero real
      })
      .eq('id', orderId);
    ...
    if (!error) {
      ...
      if (telMatch) {
        const msg = `*AMOREE - Confirmación de Pesos* 🥑%0A%0A` +
                    `¡Hola! Ya pesamos tu pedido en tienda con el peso real:%0A` +
                    `--------------------------%0A` +
                    orderToUpdate.detalle_pedido.map((i:any) => 
                      `- ${i.nombre}: ${i.quantity}${i.unidad || 'kg'} = *${formatCurrency(i.quantity * (i.precio_venta || i['$ VENTA']))}*`
                    ).join('%0A') +
                    `%0A--------------------------%0A` +
                    `*TOTAL FINAL: ${formatCurrency(orderToUpdate.total)}*%0A%0A` +
                    `Favor de enviar comprobante de pago. ¡Gracias! 🚀`;
        
        window.open(`https://wa.me/52${telMatch[1]}?text=${msg}`, '_blank');
      }
      fetchOrders(); // Refrescar para confirmar que en DB ya cambió
    } else {
  ...
  const updateStatus = async (orderId: number, nextStatus: string, order: any) => {
    ...
    if (!error) {
      if (nextStatus === 'Finalizado') {
        ...
        if (tel) {
          const msg = `*AMOREE - ¡Entregado!* 🥑%0A%0A¡Muchas gracias por tu compra, *${order.nombre_cliente || 'Cliente'}*! Tu pedido ha sido entregado exitosamente. ¡Nos vemos pronto! 🚀`;
      ...
      <div className="bg-black/90 p-6 border-b border-white/10 flex justify-between items-center sticky top-0 z-50">
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl gap-1">
          {[{ id: 'orders', label: 'Pedidos' }, { id: 'pos', label: 'Terminal' }, { id: 'clients', label: 'Cartera' }, { id: 'stats', label: 'Métricas' }].map(v => (
            <button key={v.id} onClick={() => setView(v.id as any)} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase ${view === v.id ? 'bg-white text-black' : 'text-gray-500'}`}>{v.label}</button>
          ))}
        </div>
      </div>
        ...
        {view === 'orders' ? (
          <>
            <div className="flex flex-col md:flex-row gap-4 mb-10">
              <input type="text" placeholder="Buscar cliente..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1 bg-white/5 border border-white/10 px-8 py-5 rounded-[2rem] text-sm outline-none focus:border-green-500" />
              <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="bg-black border border-white/10 px-8 py-5 rounded-[2rem] text-sm font-black text-gray-400 outline-none">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {orders.filter(o => (o.nombre_cliente || '').toLowerCase().includes(searchTerm.toLowerCase()) && (statusFilter === 'Todos' || o.estado === statusFilter)).map(order => (
                <div key={order.id} className="bg-[#0A0A0A] border border-white/5 rounded-[45px] p-10 shadow-2xl">
                  ...
                  <div className="space-y-4 mb-8">
                    {order.estado === 'Pendiente' && (
                      <div className="bg-amber-500/10 border border-amber-500/20 p-4 rounded-3xl mb-4 text-center">
                        <p className="text-[10px] font-black text-amber-500 uppercase tracking-widest">⚖️ AJUSTAR PESO REAL EN BÁSCULA</p>
                      </div>
                    )}
                    {order.detalle_pedido?.map((item: any) => (
                      <div key={item.sku || item.SKU} className="flex justify-between items-center bg-white/[0.02] p-4 rounded-2xl border border-white/5">
                        ...
                        {order.estado === 'Pendiente' ? (
                          <div className="flex items-center gap-2">
                            <input 
                              ...
                              className="w-20 bg-black border border-white/10 text-center rounded-xl py-2 text-sm font-black text-green-500 outline-none focus:border-green-500" 
                            />
                            <span className="text-[10px] font-black text-gray-600 uppercase">{item.unidad || 'kg'}</span>
                          </div>
                        ) : (
                          <span className="text-xs font-black">{item.quantity}{item.unidad || 'kg'} - {formatCurrency(item.quantity * (item.precio_venta || item['$ VENTA']))}</span>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex justify-between items-center pt-8 border-t border-white/5">
                    <div>
                      <p className="text-[10px] font-black text-gray-600 uppercase mb-1">Monto de Cobro</p>
                    ...
                    <div className="flex flex-col gap-3">
                      {order.estado === 'Pendiente' && (
                        <button 
                          onClick={() => handleConfirmWeights(order.id)} 
                          className="bg-green-600 text-white px-10 py-5 rounded-[25px] text-[10px] font-black uppercase shadow-lg shadow-green-600/20 hover:scale-105 transition-all"
                        >
                          ⚖️ Confirmar Pesos
                        </button>
                      )}
                      ...
                      {order.estado === 'Pendiente de Pago' && (
                        <div className="flex flex-col gap-2">
                          <select 
                            ...
                            className="bg-white/5 border border-white/10 rounded-xl p-2 text-[10px] font-black uppercase outline-none"
                          >
                          ...
                          <button onClick={() => updateStatus(order.id, 'Pagado - Por Entregar', order)} className="bg-blue-600 text-white px-10 py-5 rounded-[25px] text-[10px] font-black uppercase shadow-lg shadow-blue-600/20">💰 Liquidar</button>
                        </div>
                      )}
                      
                      {order.estado === 'Pagado - Por Entregar' && (
                        <button onClick={() => updateStatus(order.id, 'Finalizado', order)} className="bg-white text-black px-10 py-5 rounded-[25px] text-[10px] font-black uppercase shadow-lg">📦 Entregado</button>
                      )}
                    </div>
                  </div>
                </div>
