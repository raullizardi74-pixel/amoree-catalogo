...
import { 
  ...
  Zap, Brain, Target, Package, ShoppingCart, Save, Calendar as CalendarIcon, Check, AlertCircle
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      const { error } = await supabase
        .from('app_settings')
        .upsert({ id: 'directivas_ia', content: notasIA, updated_at: new Date() });
  ...
  const engine = useMemo(() => {
    ...
    // Reparación de lógica de fechas para evitar bloqueos
    try {
      if (rango === 'hoy') { inicio = startOfDay(ahora); fin = endOfDay(ahora); }
        ...
        if (!isValid(inicio) || !isValid(fin)) { inicio = startOfDay(ahora); fin = endOfDay(ahora); }
      }
    } catch (e) { inicio = startOfDay(ahora); fin = endOfDay(ahora); }
    ...
    const comprasDetalle = cRange.map(compra => {
      ...
      return { ...compra, diffAnterior: ((compra.costo_unitario - precioAnterior) / (precioAnterior || 1)) * 100, diffPromedio: ((compra.costo_unitario - promedio6) / (promedio6 || 1)) * 100 };
    });
  ...
  if (loading) return <div className="min-h-screen bg-black flex items-center justify-center text-green-500 font-black animate-pulse tracking-[0.3em]">RECONECTANDO SISTEMAS...</div>;
    ...
    <div className="bg-[#050505] min-h-screen p-4 md:p-10 text-white font-sans overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 bg-[#0A0A0A] p-6 rounded-[35px] border border-white/5">
        <div>
           ...
           <p className="text-[7px] font-bold text-gray-600 uppercase tracking-[0.4em]">Socio Estratégico: Automatiza con Raúl</p>
        </div>
      ...
      <div className="flex overflow-x-auto gap-2 pb-6 mb-8 no-scrollbar sticky top-0 bg-[#050505] z-50">
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-6 py-4 rounded-[22px] text-[9px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-white text-black' : 'bg-white/5 text-gray-500'}`}>
            {tab.icon} {tab.label}
          </button>
        ))}
      </div>
        ...
        {activeTab === 6 && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-blue-600/10 border border-blue-500/20 p-8 rounded-[40px] flex justify-between items-center shadow-lg">
               ...
               <p className="text-2xl font-black text-white">{engine.cRange.length} ITEMS</p>
            </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-in fade-in duration-500">
              {engine.chartCategorias.map(cat => (
                <div key={cat.name} className="bg-[#0A0A0A] p-8 rounded-[40px] border border-white/5">
                   ...
                   <p className="text-[8px] font-bold text-gray-700 uppercase italic">Objetivo: {GOALS[cat.name]}%</p>
                </div>
              ))}
           </div>
        )}
