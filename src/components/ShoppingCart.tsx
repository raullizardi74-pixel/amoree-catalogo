...
export default function ShoppingCart() {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-[200] transition-all duration-500 ${isOpen ? 'h-full' : 'h-20'}`}>
        ...
        {!isOpen && (
          <div onClick={() => cartItems.length > 0 && setIsOpen(true)} className="h-20 px-6 flex justify-between items-center cursor-pointer">
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-2xl ${cartItems.length > 0 ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-400'}`}><ShoppingBasket size={24} /></div>
              <div>
            ...
            {cartItems.length > 0 && <ChevronUp className="text-gray-400" />}
          </div>
        )}

        {/* ✅ VISTA EXPANDIDA CON BOTÓN "SEGUIR COMPRANDO" ULTRA-VISIBLE */}
        {isOpen && (
          <div className="flex flex-col h-full">
            
            {/* BARRA SUPERIOR DE ACCIÓN */}
            <div className="bg-green-600 px-6 py-5 flex justify-between items-center shrink-0 shadow-lg">
              <button 
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-2 text-[11px] font-black uppercase text-white tracking-[0.1em] active:scale-95 transition-all"
              >
                <ArrowLeft size={18} strokeWidth={3}/> ← SEGUIR COMPRANDO
              </button>
              <button onClick={() => setIsOpen(false)} className="bg-white/20 p-2 rounded-full text-white">
                <X size={22} />
              </button>
            </div>

            <div className="flex-1 overflow-y-auto px-6 pt-6 pb-32 no-scrollbar bg-gray-50">
                   ...
                   return (
                    <div key={item.sku} className="bg-white border border-gray-100 p-4 rounded-3xl flex justify-between items-center shadow-sm">
                      ...
                      <div className="flex items-center bg-gray-100 rounded-2xl p-1 gap-3">
                        <button onClick={() => decrementCartItem(item.sku, step)} className="w-8 h-8 flex items-center justify-center text-red-500">
                        ...
                        <button onClick={() => addToCart(item, step)} className="w-8 h-8 flex items-center justify-center text-green-600">
              ...
              {/* DATOS Y TOTALES */}
              <div className="bg-white rounded-[2.5rem] p-6 border border-gray-200 mb-8 space-y-4 shadow-sm">
                {!user && <input type="text" placeholder="NOMBRE COMPLETO" value={guestName} onChange={(e) => setGuestName(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl py-3 px-4 text-sm font-black outline-none" />}
                <input type="tel" placeholder="WHATSAPP (10 DÍGITOS)" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl py-3 px-4 text-sm font-black outline-none" />
                <div className="grid grid-cols-2 gap-4">
                  <DatePicker selected={deliveryDate} onChange={(date: Date) => setDeliveryDate(date)} minDate={new Date()} dateFormat="dd/MM/yyyy" locale="es" className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl py-2 px-3 text-[10px] font-black" />
                  <select value={deliveryTime} onChange={(e) => setDeliveryTime(e.target.value)} className="w-full bg-gray-50 border-2 border-gray-100 rounded-2xl py-2 px-3 text-[10px] font-black">
              ...
              <div className="px-2 space-y-2 mb-10">
                ...
                <div className="flex justify-between items-center pt-4 border-t-2 border-dashed border-gray-200">
                  <span className="text-2xl font-black text-green-900">TOTAL</span>
                  <span className="text-3xl font-black text-green-900">{formatCurrency(totalFinal)}</span>
                </div>
