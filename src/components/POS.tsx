...
import { 
  ...
  ShieldOff, Scale, Zap, UserPlus, Receipt, Camera
} from 'lucide-react';
...
export default function POS({ onBack }: { onBack: () => void }) {
  ...
  const finalizarVenta = async () => {
    ...
    try {
      ...
      // ✅ PAYLOAD REVISADO (Formato CSV de pedidos)
      const payload = {
        usuario_email: user?.email || null, // Usamos null para evitar errores de validación de correo
        ...
        pago_confirmado: true, // Columna detectada en el CSV
        detalle_pedido: cart.map(i => ({ id: i.id, sku: i.sku, nombre: i.nombre, quantity: i.quantity, price: i.precio_venta, unidad: i.unidad }))
      };

      // 1. Inserción con Debugger Forzado
      const { data, error: errP } = await supabase.from('pedidos').insert([payload]).select();

      if (errP) {
        // ❌ ESTO NOS DIRÁ EL ERROR REAL
        throw new Error(`[ERROR DB] ${errP.code}: ${errP.message}\nDetalle: ${errP.details}`);
      }

      if (!data || data.length === 0) {
        throw new Error("Supabase no devolvió datos. El registro podría estar bloqueado por RLS.");
      }
    ...
    <div className="h-screen bg-black text-white font-sans flex flex-col overflow-hidden">
      <div className="p-3 border-b border-white/5 flex justify-between items-center bg-[#050505] shrink-0">
        ...
        <div className="bg-green-600/10 border border-green-500/20 px-3 py-1.5 rounded-lg flex items-center gap-2">
          <Zap size={12} className="text-green-500"/><span className="text-[8px] font-black uppercase text-green-400">Tablet Mode v5</span>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 flex flex-col p-4 gap-4 overflow-hidden border-r border-white/5">
          <div className="relative shrink-0">
            ...
            <input ref={searchInputRef} type="text" placeholder="BUSCAR..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#0A0A0A] border border-white/10 rounded-2xl py-4 pl-14 pr-6 font-black uppercase text-xs outline-none focus:border-green-500" />
          </div>

          <div className="grid grid-cols-4 xl:grid-cols-5 gap-2 overflow-y-auto no-scrollbar flex-1 pb-10">
            {filteredProducts.map(p => (
              <button key={p.id} onClick={() => addToCart(p)} className={`p-3 rounded-[25px] border text-left h-32 flex flex-col justify-between ${p.stock_actual <= 0 ? 'opacity-20 grayscale' : 'bg-[#0A0A0A] border-white/5 hover:border-green-500 shadow-lg'}`}>
        ...
        <div className="w-[340px] flex flex-col bg-[#050505] p-5 shadow-2xl shrink-0">
          ...
          <div className="border-t border-white/5 pt-4">
            ...
            <button onClick={() => setShowPaymentModal(true)} disabled={cart.length === 0} className="w-full py-5 bg-green-600 text-black rounded-2xl font-black uppercase text-[10px] hover:bg-green-500 shadow-xl">Continuar</button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/98 backdrop-blur-2xl">
          <div className="bg-[#0A0A0A] border border-white/10 rounded-[50px] p-8 w-full max-w-4xl shadow-2xl relative flex gap-10">
            ...
            <div className="flex-1">
              <h3 className="text-3xl font-black uppercase italic text-blue-500 mb-8">Pago y Cliente</h3>
              <div className="grid grid-cols-2 gap-4 mb-8">
                <button onClick={() => {setWithTicket(false); setIsAnonymous(true); setSelectedClient(null);}} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 ${!withTicket ? 'bg-white text-black' : 'bg-black border-white/5 text-gray-600'}`}><ShieldOff size={20}/><span className="text-[9px] font-black uppercase">Venta Rápida</span></button>
                <button onClick={() => {setWithTicket(true); setIsAnonymous(false);}} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 ${withTicket ? 'bg-green-600 border-green-600 text-white' : 'bg-black border-white/5 text-gray-600'}`}><Receipt size={20}/><span className="text-[9px] font-black uppercase">Con Ticket</span></button>
              </div>
              <div className="mb-8">
                <div className="flex justify-between items-center mb-2"><span className="text-[8px] font-black text-gray-500 uppercase tracking-widest">Seleccionar</span><button onClick={() => setShowNewClientForm(true)} className="text-[8px] font-black text-blue-500 flex items-center gap-1"><UserPlus size={10}/> Nuevo</button></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {['Efectivo', 'Tarjeta', 'Transferencia', 'A Cuenta'].map(m => (
                  <button key={m} disabled={isAnonymous && m === 'A Cuenta'} onClick={() => setMetodoPago(m as any)} className={`py-4 rounded-xl border flex flex-col items-center justify-center ${metodoPago === m ? 'bg-blue-600 text-white shadow-lg' : 'bg-black border-white/5 text-gray-700'}`}><span className="font-black uppercase text-[9px]">{m}</span></button>
                ))}
              </div>
            </div>
            ...
            <div className="w-[340px] bg-white/[0.02] border border-white/5 rounded-[40px] p-8 flex flex-col justify-between">
                <div>
                  <div className="text-center mb-8"><p className="text-[9px] font-black text-gray-500 uppercase">A Cobrar</p><p className="text-4xl font-black italic tracking-tighter text-white">{formatCurrency(totalVenta)}</p></div>
                  {metodoPago === 'Efectivo' && (
                    <div className="space-y-4">
                      ...
                      <div className="text-center py-4 bg-green-500/5 rounded-2xl border border-green-500/10"><p className="text-[8px] font-black text-green-500 uppercase mb-1">Cambio:</p><p className="text-3xl font-black text-white">{formatCurrency(cambio)}</p></div>
                    </div>
                  )}
                </div>
                <button onClick={finalizarVenta} disabled={isPaymentDisabled} className={`w-full py-6 rounded-[25px] font-black uppercase text-[10px] shadow-2xl transition-all ${isPaymentDisabled ? 'bg-white/10 text-gray-500 cursor-not-allowed' : 'bg-white text-black active:scale-95'}`}>
      ...
      {showNewClientForm && (
        <div className="fixed inset-0 z-[300] bg-black/95 flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-[#0A0A0A] border border-white/10 p-8 rounded-[40px] w-full max-w-sm">
            <h3 className="text-xl font-black uppercase italic mb-6">Nuevo Cliente</h3>
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4"><button onClick={() => setShowNewClientForm(false)} className="flex-1 bg-white/5 py-4 rounded-xl text-[10px] font-black uppercase">Cerrar</button><button onClick={crearNuevoCliente} className="flex-1 bg-blue-600 py-4 rounded-xl text-[10px] font-black uppercase text-white">Guardar</button></div>
            </div>
          </div>
        </div>
      )}
