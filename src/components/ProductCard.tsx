    ...
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:scale-105 transition-transform duration-300">
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between items-center mt-4">
          {isEditing ? (
            <div className="flex items-center gap-2">
              <input
                ...
                autoFocus
              />
              <button onClick={handleUpdatePrice} className="text-xs bg-blue-600 text-white px-2 py-1 rounded">
                OK
              </button>
            </div>
          ) : (
          ...
          {quantity === 0 ? (
            <button onClick={() => addToCart(product)} className="bg-green-500 text-white font-bold py-2 px-4 rounded hover:bg-green-600 transition-colors">
              +
            </button>
          ) : (
            <div className="flex items-center gap-2">
              <button onClick={() => decrementCartItem(product.sku)} className="bg-gray-200 text-gray-700 font-bold py-1 px-3 rounded hover:bg-gray-300 transition-colors">-</button>
              <span>{quantity}</span>
              <button onClick={() => addToCart(product)} className="bg-gray-200 text-gray-700 font-bold py-1 px-3 rounded hover:bg-gray-300 transition-colors">+</button>
            </div>
          )}
        </div>
      </div>
